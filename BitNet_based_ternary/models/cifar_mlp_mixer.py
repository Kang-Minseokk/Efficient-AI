...
from get_args import get_args  # 이게 누락되어 있었습니다

args = get_args()

if args.quantize_type == 'binary':
    print("Cifar MLPMixer - Using Binary Linear")
    LinearLayer = BinaryLinear
    # ConvLayer = BinaryConv1d
    
elif args.quantize_type == 'qat_ternary':
    print("Cifar MLPMixer - Using Ternary Linear")
    LinearLayer = TernaryLinear
    # ConvLayer = TernaryConv1d
    
elif args.quantize_type == 'trick_ternary':
    print("Cifar MLPMixer - Using Tricky Ternary Linear")
    LinearLayer = TernaryTrickLinear
    # ConvLayer = TernaryTrickConv1d
    
else:
    print("Cifar MLPMixer - Using FP Linear")
...
class MLPMixer(nn.Module):
    def __init__(self,in_channels=3,img_size=32, patch_size=4, hidden_size=512, hidden_s=256, hidden_c=2048, num_layers=8, num_classes=10, drop_p=0., off_act=False, is_cls_token=False):
        ...
        self.clf = LinearLayer(hidden_size, num_classes)
...
class MLP2(nn.Module):
    def __init__(self, hidden_size, hidden_c, drop_p, off_act):
        ...
        self.ln = nn.LayerNorm(hidden_size)
        self.fc1 = LinearLayer(hidden_size, hidden_c)
        self.do1 = nn.Dropout(p=drop_p)
        self.fc2 = LinearLayer(hidden_c, hidden_size)
        self.do2 = nn.Dropout(p=drop_p)
        self.act = F.gelu if not off_act else lambda x:x
    def forward(self, x):
        out = self.do1(self.act(self.fc1(self.ln(x))))
