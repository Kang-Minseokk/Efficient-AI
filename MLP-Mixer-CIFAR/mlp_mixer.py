...
from models.tricky_ternary import TernaryTrickLinear, TernaryTrickConv1d, TernaryTrickConv2d
from models.bit1_58 import TernaryLinear, TernaryConv1d, TernaryConv2d
...
class MLPMixer(nn.Module):
    def __init__(self,in_channels=3,img_size=32, patch_size=4, hidden_size=512, hidden_s=256, hidden_c=2048, num_layers=8, num_classes=10, drop_p=0., off_act=False, is_cls_token=False):
        ...
        self.patch_emb = nn.Sequential(
            TernaryTrickConv2d(in_channels, hidden_size ,kernel_size=patch_size, stride=patch_size),
...
class MLP1(nn.Module):
    def __init__(self, num_patches, hidden_s, hidden_size, drop_p, off_act):
        ...
        self.ln = nn.LayerNorm(hidden_size)
        self.fc1 = TernaryTrickConv1d(num_patches, hidden_s, kernel_size=1)
        self.do1 = nn.Dropout(p=drop_p)
        self.fc2 = TernaryTrickConv1d(hidden_s, num_patches, kernel_size=1)
...
class MLP2(nn.Module):
    def __init__(self, hidden_size, hidden_c, drop_p, off_act):
        ...
        self.ln = nn.LayerNorm(hidden_size)
        self.fc1 = TernaryTrickLinear(hidden_size, hidden_c) # Linear Layer
        self.do1 = nn.Dropout(p=drop_p)
        self.fc2 = TernaryTrickLinear(hidden_c, hidden_size) # Linear Layer
        self.do2 = nn.Dropout(p=drop_p)
        self.act = F.gelu if not off_act else lambda x:x
    def forward(self, x):
        out = self.do1(self.act(self.fc1(self.ln(x))))
